...
int main(void) {
  ...
  while(count > 0 && !end) {
    ...
    for(i = 0; i < 5; i++) {
      ...
      } else if((x == 0 && y == -1) || (x == (GRIDX - 1) && y == GRIDY) || (x >= 0 && x < GRIDX && y >= 0 && y < GRIDY)) {
        if((x == 0 && y == -1) || (x == (GRIDX - 1) && y == GRIDY) || grid->cell[x][y] == 0) {
  ...
  printf("Minutes to reach the end, go back, and return (sodding elves...) = %d\n", end); // 899
  return 0;
}
