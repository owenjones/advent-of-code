...
int main(void) {
  ...
  int n, answer = 0, remaining = 100;
  while(!answer) {
    fscanf(fptr, "%i,", &n);
    for(int i = 0; i < 100; i++) {
      mark_board(boards[i], n);
      if(test_board(boards[i])) remaining--;
      if(remaining == 0) {
        answer = get_board_score(boards[i], n);
        break;
      }
    }
  }
  ...
  printf("Last winning board score = %i\n", answer);
  return 0;
}
