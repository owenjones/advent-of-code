...
var steps = 1
var cwPath: [(Int, Int)] = [nodes[0]]
var ccwPath: [(Int, Int)] = [nodes[1]]
...
// Step 4: get all the pipe tiles within the path
var path = [start]
path.append(contentsOf: cwPath)
path.append(contentsOf: ccwPath)

// Step 5: walk each row and sum the tiles which fall within the loop
var tiles = 0
for (x, row) in map.enumerated() {
  var within = false
  for (y, char) in row.enumerated() {
    if path.contains(where: { $0 == (x, y) }) {
      if ["|", "L", "J", "S"].contains(char) {
        within = !within
      }
    }
    else {
...
print("Number of tiles enclosed by the loop: \(tiles)")