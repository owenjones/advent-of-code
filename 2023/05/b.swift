...
let raw = input[0].wholeMatch(of: #/seeds: (.*)/#)!.1.split(separator: " ").map{ Int($0) ?? 0 }
var seeds: [(Double, Double)] = []
for i in stride(from: 0, to: raw.count, by: 2) {
  seeds.append((Double(raw[i]), Double(raw[i] + raw[i + 1] - 1)))
}
...
for range in seeds {
  ...
  for piece in optimusPrime.pieces {
    ...
    let lower = max(piece.start, range.0)
    let location = Double(lower + piece.offset)
    locations.append(location)
  }

  lowest = min(Double(locations.min()!), lowest)
}

print("Lowest location corresponding to an initial seed: \(Int(lowest))")